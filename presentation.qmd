...
-   **"Classical" IDV systems (~1980-2010s)**
    - Standalone apps
    - Deep interactivity (e.g. linked selection, parametric interaction, tours,...) 
...
- Starting from the *plots as partitions* model, we can be ever more precise
- We can show that the fundamental structure is a *monoid*: a set equipped with an associative binary operation and a neutral element
- Then, aggregation is a functor from the partition category to the category of summaries (preserves set union)
  - Known in parallel computing and relational database research [@parent2018; @fegaras2017; @lin2013; @gibbons2018]

## Monoids, groups, and beyond

- Monoids are fundamental for modeling geoms as sets under union
- To preserve other properties, we need more structure
- For instance, to preserve set difference, the operation needs to have an inverse: 
  - Algebraic *groups* (i.e. the ones from group theory)
  - Practical impact on interactive graphics: monoids are enough for showing *single-subset* linked selection, groups necessary for showing selection of *multiple independent groups* 
  - Comparing $A$ vs. $A \cup B$, or comparing $A$ vs. $B$ directly
- Other properties: monotonicity, commutativity, etc...
  
## Closing thoughts...

- Components of graphics are not independent, but instead deeply linked
- Missing link in GoG, can be described using the language of category theory 
- Most novel contribution, many interesting applied problems as well:
  - Features: interactive and otherwise
  - Data visualization pipeline: reactivity, hierarchical summaries, scales, etc... 
  - Architecture and programming paradigm  

## Feel lucky to have stumbled upon an interesting topic, a lot of room for future work
