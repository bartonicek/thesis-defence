...
-   **Modern web-based IDV systems (2005-current)** 
    - Focus on presentation, dashboards
    - Highly customizable but less interactivity out-of-the-box
    - Compositional style: *plot with point geometry and continuous x- and y- encodings...* (inspired by GoG)
    
## *Why is it difficult to combine deep interactive features with the flexibility of GoG?* {.center-title}
...
- *Poweful, plots can be specified via simple declarative schemas*
- *However, are there limits?*
...
## Going further: Plots as partitions

- Useful idea: plots as *partitions*:
...
- The data partition can have a hierarchical structure
  ...
  - *Not purely visual* - segments represent data
...
- Not always the case:
  - E.g. stacking means leads to malformed plots  
...
- *Only* sums and counts? *Why* can we stack some statistics but not others?
  
## Child FEV: Barplot of maximums

```{r}
fev <- read.csv("./fev.csv")
fev$smoke <- factor(fev$smoke, labels = c("All kids", "Smokers"))
...
ggplot(fev2, aes(age, fev, fill = smoke)) +
  geom_bar(stat = "identity", position = "identity", col = "white") +
  scale_fill_manual(values=c("grey80", "steelblue")) +
  labs(x = "Age", y = "Max FEV", fill = NULL)
```
...
- Most novel contribution, many interesting applied problems as well:
  ...
  - Data visualization pipeline: reactivity, partitioning, summaries, scales, etc... 
